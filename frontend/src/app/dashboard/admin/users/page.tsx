    ...
    <AuthGuard>
      <div className="space-y-6 px-6 py-6">
        ...
        {isAdmin && <section className="rounded-lg border border-[var(--border)] bg-[color:var(--bg-panel)] p-4">
          ...
          {createdInvite && <div className="mt-4 rounded-md border border-cyan-500/30 bg-cyan-950/20 p-3 text-xs">{createdInvite.email_sent ? <p>Invitation email sent to {createdInvite.email}.</p> : <><p>Email delivery is not configured. Share this testing link securely:</p><p className="mt-2 break-all text-cyan-300">{createdInvite.invite_url}</p></>}</div>}
        </section>}
